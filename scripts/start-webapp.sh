...
BACKEND_PORT=9000
FRONTEND_PORT=3001
PROJECT_DIR="/home/major/Desktop/aetherlink"
...
if ps -p $FRONTEND_PID > /dev/null; then
    ...
    print_info "Frontend logs: $PROJECT_DIR/frontend.log"
...
echo -e "  Or use:        pkill -f hypercorn; pkill -f vite"
echo ""
echo -e "${YELLOW}Note: Processes are running in background. Close this terminal safely.${NC}"
echo -e "${YELLOW}Hardware initialization may take 10-15 seconds. Check backend.log if needed.${NC}"
echo ""

# Save PIDs for easy stopping later
echo "$BACKEND_PID" > .webapp_backend.pid
echo "$FRONTEND_PID" > .webapp_frontend.pid
