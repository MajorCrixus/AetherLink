...
if [ -f ".webapp_backend.pid" ]; then
    BACKEND_PID=$(cat .webapp_backend.pid)
    kill_process "$BACKEND_PID" "Backend"
    rm -f .webapp_backend.pid
else
    echo -e "  ${BLUE}ℹ${NC} No saved backend PID"
fi

if [ -f ".webapp_frontend.pid" ]; then
    FRONTEND_PID=$(cat .webapp_frontend.pid)
    kill_process "$FRONTEND_PID" "Frontend"
    rm -f .webapp_frontend.pid
else
